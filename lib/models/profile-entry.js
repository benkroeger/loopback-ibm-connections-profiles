...
module.exports = function (ProfileEntryModel) {
  ProfileEntryModel.on('dataSourceAttached', function () {
    ...
    ProfileEntryModel.photo = function (context, userid, callback) {
      if (userid === 'me') {
        profiles.getServiceDocument({
          userid: undefined
        }, pickRequestData(context.req), function (err, serviceDocument) {
          if (err) {
            return callback(err);
          }

          request.get(settings.baseUrl + '/photo.do', _.merge({}, pickRequestData(context.req), {
            qs: {
              userid: serviceDocument.userid
            }
          })).pipe(context.res);
